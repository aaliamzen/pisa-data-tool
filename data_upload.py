...
def dataframe_to_html_with_tooltips(df, variable_labels, value_labels, title="DataFrame", scrollable=False):
    ...
    for col in df.columns:
        ...
        header_name = "Gender" if col == "ST004D01T" else col
        header_row += f'<th><div class="tooltip">{header_name}<span class="tooltiptext">{description}</span></div></th>'
    data_rows = ""
...
with col1:
    st.markdown("""
    **PisaSplitter.exe** — Splits large PISA SPSS `.sav` files by country (CNTRYID).  
    ...
    try:
        with open("assets/PisaSplitter.exe", "rb") as file:
            st.download_button(
                ...
                data=file,
                file_name="PisaSplitter.exe",
                mime="application/octet-stream",
                help="Download the file splitter tool"
            )
    except FileNotFoundError:
        st.error("PisaSplitter.exe not found in the `assets/` folder. Please upload it to your repository.")
    except Exception as e:
        st.error(f"Error accessing download file: {e}")
...
if df is None:
    ...
    excluded_variables = [
        ...
        'Option_WBQ', 'Option_LDW', 'Option_FLA', 'BICT_P1', 'BICT_P2', 'BICT_S', 'MALE', 'ST001D01T_3'
    ]
