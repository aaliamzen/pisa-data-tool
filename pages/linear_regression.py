    ...
    try:
        ...
        for idx, var in enumerate(['Intercept'] + var_names):
            ...
            if idx == 0:  # Intercept
                ...
                if np.isnan(sd_y) or np.isnan(sd_x[idx-1]) or sd_x[idx-1] == 0:
                    std_coef = np.nan
                else:
        ...
        return results, r_squared, r_squared_adj
    except Exception as e:
        st.error(f"Error in weighted_ols_regression: {str(e)}")
        return [(var, np.nan, np.nan, np.nan, np.nan) for var in ['Intercept'] + var_names], np.nan, np.nan

# Function to compute BRR standard errors for regression coefficients
def compute_brr_se_regression(x, y, replicate_weights, reg_data, progress_bar, var_names):
    try:
        # Initial regression with final student weights for reference
        main_results, main_r_squared, main_r_squared_adj = weighted_ols_regression(x, y, reg_data['W_FSTUWT'].values, var_names)
        ...
        for idx, weight_col in enumerate(replicate_weights):
            ...
            rep_results, _, _ = weighted_ols_regression(x_rep, y_rep, w_rep, var_names)
...
def apply_rubins_rules_regression(all_results, n, var_names):
    try:
        # Separate results, R-squared, and adjusted R-squared values
        ...
        for idx, var_result in enumerate(var_results):
            ...
            combined_std_coef = np.nanmean(std_coefs_list)
        ...
        return combined_results, r_squared, r_squared_adj
    except Exception as e:
        st.error(f"Error in apply_rubins_rules_regression: {str(e)}")
        return [(var, np.nan, np.nan, np.nan, np.nan) for var in ['Intercept'] + var_names], np.nan, np.nan

# Function to compute linear regression with PVs and BRR
def compute_linear_regression_with_pvs(df, dependent_var, independent_vars, weights, replicate_weights, use_brr, var_labels):
    try:
        ...
        if not dep_is_pv and not any(indep_is_pv):
            ...
            # Run regression
            results, r_squared, r_squared_adj = weighted_ols_regression(X, y, w, independent_vars)
            if use_brr:
                st.write("Calculating standard errors using replicate weights...")
                brr_progress = st.progress(0)
                brr_se, brr_se_std = compute_brr_se_regression(X, y, replicate_weights, data, brr_progress, independent_vars)
        ...
        for pv_idx in pv_nums:
            ...
            st.write(f"Processing regression with {pv_var}...")
            ...
            # Run regression
            results, r_squared, r_squared_adj = weighted_ols_regression(X, y, w, indep_vars)
            if use_brr:
                st.write("Calculating standard errors using replicate weights...")
                brr_progress = st.progress(0)
                brr_se, brr_se_std = compute_brr_se_regression(X, y, replicate_weights, data, brr_progress, indep_vars)
                # Update results with BRR standard errors and recompute p-values
                updated_results = []
                for idx, (var, coef, std_coef, _, _) in enumerate(results):
                    se = brr_se[idx]
            ...
            all_results.append((results, r_squared, r_squared_adj))
        ...
        st.write("Combining results across plausible values...")
        combined_results, combined_r_squared, combined_r_squared_adj = apply_rubins_rules_regression(all_results, len(data), indep_var_labels)
        
        return combined_results, combined_r_squared, combined_r_squared_adj
    except Exception as e:
...
def render_regression_table(dependent_var_label, results, r_squared, r_squared_adj):
    html_content = """
    <style>
    .reg-table-container {
    ...
    <div class="reg-table-container">
        ...
        <div class="reg-table-note"><i>Note.</i> <i>R²</i> = {{r_squared}}, <i>Adjusted R²</i> = {{r_squared_adj}}</div>
    </div>
    """
    data_rows = ""
    for idx, (var, coef, std_coef, se, p_value) in enumerate(results):
    ...
    full_html = html_content.replace("{{dependent_var}}", dependent_var_label).replace("{{data_rows}}", data_rows).replace("{{r_squared}}", r_squared_display).replace("{{r_squared_adj}}", r_squared_adj_display)
    return full_html
...
def create_word_table(dependent_var_label, results, r_squared, r_squared_adj):
    doc = Document()
    ...
    # Add "Adjusted R²" (italicized)
    run = note.add_run("Adjusted R²")
    run.font.name = 'Times New Roman'
    run.font.size = Pt(10)
    run.italic = True
        ...
        if run_analysis and independent_vars:
            try:
                if 'W_FSTUWT' not in df.columns:
                    ...
                    # Compute regression with PVs and BRR, passing label_to_var for mapping
                    results, r_squared, r_squared_adj = compute_linear_regression_with_pvs(df, dependent_var, independent_vars, df['W_FSTUWT'], replicate_weight_cols, use_brr, label_to_var)
                    ...
                    st.write("Rendering regression table...")
                    table_html = render_regression_table(dependent_var_label, results, r_squared, r_squared_adj)
                    ...
                    doc_buffer = create_word_table(dependent_var_label, results, r_squared, r_squared_adj)
                    st.download_button(
