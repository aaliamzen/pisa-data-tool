...
st.caption(
    "Tests whether a mediator (M) carries part of the association from a predictor (X) to an outcome (Y). "
    "Three weighted OLS models are estimated: M ~ X; Y ~ X + M; Y ~ X. "
    "The indirect effect is a × b, with a Sobel test. "
    "Y may be a PISA score domain; X and M should be observed scales or items (not score domains). "
    "This is a single-level weighted approximation, not multilevel SEM."
)
if df is None or df.empty:
    ...
    if len(unique_var_labels) < 2:
        ...
        covariate_options = [
            ...
            if lab not in (dependent_var_label, x_label, m_label) and lab
        ]
        default_covariates = []
